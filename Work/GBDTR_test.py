...
def train_useGBDT(data,options):
    reg = GradientBoostingClassifier(n_estimators=int(options['n_estimators']),
                               learning_rate=options['learning_rate'],
                               max_depth=options['max_depth'],
                               max_leaf_nodes=options['max_leaf_nodes']
                               )
    reg.fit(data['X_train'],data['y_train'])
    y_pred = reg.predict(data['X_valid'])
    #r2=r2_score(data['y_valid'],y_pred)
...
def make_facies_log_plot(logs):
    ...
    plt.savefig('Gbdt_Facies_fig.png')
    img = Image.open('Gbdt_Facies_fig.png')
    img = img.transpose(Image.ROTATE_270)
    img.save("Gbdt_Facies_fig_270.png")
# if __name__ == '__main__':
#     options = {'n_neighbors': 4,
